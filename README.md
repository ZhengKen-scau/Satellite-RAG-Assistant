...
### 为什么选择自定义RAG实现而非LangChain？

**设计理念**: 
- **轻量化**: 避免LangChain复杂依赖，专注于核心RAG功能
- **可控性**: 完全掌握检索、提示词构建、生成每个环节
- **性能优化**: 直接调用底层库，减少抽象层开销
- **专业定制**: 针对遥感领域需求进行深度优化

**实现优势**:
- **代码简洁**: 核心逻辑清晰易懂，便于维护和扩展
- **资源高效**: 内存占用更低，启动速度更快
- **错误处理**: 自定义完善的异常处理和降级机制
- **学习成本低**: 无需理解LangChain复杂概念，直接面向业务需求
